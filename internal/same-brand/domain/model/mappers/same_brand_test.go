...
func TestMapProductInformationToSameBrandItem(t *testing.T) {
	productInfo := sharedModel.ProductInformation{
		...
		OfferPrice:        85.00,
		// Outstanding no está en ProductInformation, se espera valor por defecto false
		RequirePrescription: true,
		Sales:               50,
		Spaces:              5,
		Status:              "A", // Activo
		TaxRate:             19,
		// TotalStock se calcula a partir de StoresWithStock
		// QuantitySold no está en ProductInformation, se espera valor por defecto 0
		Classification: []sharedModel.Classification{
			{ID: 10, Description: "Cat1"},
			{ID: 20, Description: "Cat2"},
		},
		// ExpressWithSubscription no está en ProductInformation, se espera valor por defecto false
		// PosGroup se espera "100" por defecto
		...
		// DeliveryPrice se espera 0 por defecto
		DesHiddenSEO:  func(s string) *string { return &s }("Texto SEO oculto para el producto"), // Helper para puntero a string
		OnlyOnline:    true,
		DeliveryTime:  "24-48 horas",
		StoresWithStock: []int{1001, 1002, 1003}, // Esto dará TotalStock = 3 y GlobalStock = 3
		Outofstore:    false,
		// IsFlashOffer no está en ProductInformation, se espera valor por defecto false
		OfferStartDate:    1672531200, // Ejemplo: 1 de enero de 2023
		OfferEndDate:      1675209600, // Ejemplo: 1 de febrero de 2023
		PrimePrice:        80.00,
		...
		RmsGroup:          "GR01",
		RmsSubclass:       "SC01",
		HasStock:          true, // Esto implica WithoutStock = false
		URL:               "http://example.com/product/item-123",
		ObjectID:          "algolia-obj-123", // Usado para UuidItem
	...
	expectedItem := model.SameBrandItem{
		...
		OfferPrice:                 85.00,
		Outstanding:                false,
		RequirePrescription:        "true", // convertBoolToString
		Sales:                      "50",   // strconv.FormatInt
		Spaces:                     "5",    // strconv.Itoa
		Status:                     "A",
		TaxRate:                    19,
...
// Nota: Los tests para mapToSeoData, getSEOText, mapSellerAddresses, mapDimensions
// se prueban indirectamente a través de TestMapProductInformationToSameBrandItem.
// Si su lógica fuera muy compleja, merecerían tests unitarios separados.
// Por ahora, la prueba principal del mapper completo es un buen comienzo.
