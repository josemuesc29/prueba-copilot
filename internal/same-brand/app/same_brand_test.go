...
func TestSameBrand_GetItemsBySameBrand_Success_FromAlgolia(t *testing.T) {
	...
	query := `("query":"items","filters":"(stock.fulfillment_stock.locations.store_id=1 OR stock.fulfillment_stock.locations.store_id=2 OR stock.fulfillment_stock.locations.store_id=3) AND city_name='BOGOTA' AND brand='TestBrand' AND stock>0","hitsPerPage":"24")`
